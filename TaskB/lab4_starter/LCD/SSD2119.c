...
void LCD_GPIOInit(void){
    unsigned long wait = 0;
		...
		RCGCGPIO_PORTS |= 0x2;								 // activate port B
    ...
    GPIODIR_PORTB |= 0xFF;                 // make PB0-7 outputs
		...
		RCGCGPIO_PORTS |= 0x1;								// activate port A
		wait++;                               // wait for port activation
    wait++;                               // wait for port activation
    GPIODIR_PORTA |= 0xF0;                // make PA4-7 outputs
		GPIOAFSEL_PORTA &= ~(0xFF);						// disable alternate functions
		GPIODEN_PORTA |= 0xF0;								// enable digital I/O on PA4-7
